...
export class AppComponent implements OnInit {
  ...
  // NewTask: string = "";
  tasks: { name: string, completed: boolean, starred: boolean, editMode: boolean ,minDate:string}[] = [];
  completedTasks: { name: string, completed: boolean, starred: boolean, editMode: boolean , minDate:string}[] = [];
 ...
 formatDate(date: Date): string {
  ...
  completeTask(task: { name: string, completed: boolean, starred: boolean, editMode: boolean,minDate:string }) {
  ...
  toggleStar(task: { name: string, completed: boolean, starred: boolean, editMode: boolean }) {
    task.starred = !task.starred;
    this.saveLocalStorage();
  }

  deleteTask(taskToDelete: { name: string, completed: boolean, starred: boolean, editMode: boolean }) {
    this.tasks = this.tasks.filter(task => task !== taskToDelete);
    this.completedTasks = this.completedTasks.filter(task => task !== taskToDelete);
    this.saveLocalStorage();
  }
